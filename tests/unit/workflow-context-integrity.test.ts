...
// PARTIAL. `run-store`'s proposal seed now asks the registry's own
// `isProposeTool` whether a recorded TOOL_CALL was a propose call, so a
// mock that replaces this module wholesale leaves that import undefined
// and the seed throws inside an unrelated run.
jest.mock("@/lib/mcp/tools/propose-tools", () => ({
  ...jest.requireActual("@/lib/mcp/tools/propose-tools"),
...
import { recordWorkflowContextIntegrityHalt } from "@/lib/observability/metrics";
...
const mockHaltMetric =
  recordWorkflowContextIntegrityHalt as jest.MockedFunction<any>;
    ...
    workflowStep: {
      ...
      aggregate: async ({ where }: any) => {
        const seqs = store.steps
          .filter(
            (s) => s.runId === where.runId && s.tenantId === where.tenantId,
          )
          .map((s) => s.contextSeq)
...
describe("a context that fails schema validation", () => {
  ...
  function seedRunWithInvalidContext(): string {
    ...
    const badContext = {
      input: {},
      outputs: "not-a-record" as unknown as Record<string, unknown>,
    };
    const envelope = {
  ...
  it("HALTS the run instead of continuing on a partially-valid context", async () => {
    ...
    expect(mockHaltMetric).toHaveBeenCalledWith({
  ...
  it("an UNSEALED row (no chain head at all) fails closed rather than being adopted", async () => {
    const runId = seedRunWithInvalidContext();
    const row = store.runs.get(runId)!;
    row.contextJson = JSON.stringify({
      v: CONTEXT_ENVELOPE_VERSION,
      seq: 0,
      prev: null,
      input: {},
...
describe("a context tampered with between steps", () => {
  ...
  it("is detected at the next step, and the next step never executes", async () => {
    ...
    expect(result.status).toBe("FAILED");
    expect(store.runs.get(started.runId)!.errorMessage).toContain(
  ...
  it("reports the break with digests only — never the poisoned content", async () => {
    ...
    expect(halt.detailsJson.expectedHash).not.toBe(
      halt.detailsJson.observedHash,
    );
  });
...
describe("unbounded context growth", () => {
  ...
  async function runUntilOversized() {
    mockReadTool
      ...
      .mockResolvedValueOnce(
        toolResult({ blob: MARKER + "x".repeat(MAX_CONTEXT_BYTES) }),
      );
  ...
  it("HALTS and REPORTS at the cap — it does not silently truncate", async () => {
    ...
    expect(mockHaltMetric).toHaveBeenCalledWith({
      code: "CONTEXT_SIZE_CAP_EXCEEDED",
    });
  ...
  it("leaves the last VERIFIED context intact — no trimmed blob is persisted", async () => {
    ...
    expect(Buffer.byteLength(row.contextJson, "utf8")).toBeLessThanOrEqual(
      MAX_CONTEXT_BYTES,
    );
