...
beforeEach(() => {
  ...
  mockProposeTool.mockResolvedValue({
    content: [
      { type: "text", text: JSON.stringify({ proposed: 1, quarantined: 0 }) },
    ],
  });
});
...
registerWorkflow({
  ...
  description:
    "Two reads whose corpora differ, so a constant cannot pass for a label.",
  steps: [
...
describe("content[0] is untouched — the contract every agent and workflow parses", () => {
  it("the step row and the run context carry the payload, never the envelope", async () => {
    ...
    expect(envelope.outputs).toStrictEqual({
      posture: payload,
      evidence: payload,
    });
...
describe("it fails closed", () => {
  ...
  it("a payload that is not JSON leaves a null output and an untrusted label", async () => {
    ...
    expect(store.steps.map((s) => s.outputJson)).toStrictEqual([
      "null",
      "null",
    ]);
...
describe("reading the label cannot end a run", () => {
  ...
  it.each(UNREADABLE)(
    "%s leaves a null output and an untrusted label, and the run finishes",
    async (_name, result) => {
      mockReadTool.mockResolvedValue(result);
      const started = await startWorkflowRun(ctx(), READ_WF, {});

      expect(started.status).toBe("COMPLETED");
      expect(store.steps.map((s: any) => s.status)).toStrictEqual([
        "DONE",
        "DONE",
      ]);
      expect(store.steps.map((s: any) => s.outputJson)).toStrictEqual([
        "null",
        "null",
      ]);
      expect(stepAuditRows().map((r: any) => r.provenance)).toStrictEqual([
  ...
  it("the reader itself is total — it answers untrusted rather than throwing", () => {
    ...
    expect(provenanceOfToolResult({ content: null })).toBe(
      "THIRD_PARTY_INGESTED",
    );
