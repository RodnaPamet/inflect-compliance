...
const EXEMPT_HANDLERS: readonly ExemptEntry[] = [
    ...
    { handler: 'api/mcp/token/route.ts#POST', klass: 'PROTOCOL_CREDENTIAL',
      mechanism: /exchangeMcpToken/,
      note: 'RFC 8693 exchange: the subject_token in the body IS the credential. exchangeMcpToken runs verifyApiKey, the MCP capability scope, the agent-registration gate, principal resolution and the deny-by-default tool allowlist before minting, so a token can never name a tool the agent could not have called.' },
