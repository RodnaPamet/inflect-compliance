...
const LIST_MODELS_TENANT_INDEX_SUFFICIENT: Record<string, string> = {
    ...
    AuditOutbox:
        'The drain (audit-outbox-flush) is the only findMany, and it is deliberately NOT tenant-leading: the platform-wide pass filters (status, nextAttemptAt <= now, attempts < max) so that ONE query serves every tenant, and that is covered exactly by @@index([status, nextAttemptAt]). An operator re-running a single tenant adds tenantId, served by @@index([tenantId, status]). It sits HERE rather than in LIST_QUERY_INDEXES because that layer expects a tenant-leading composite, and a tenant-leading index would be the wrong shape for the drain: the queue is empty almost always, so the selective predicate is status, not tenant. The orderBy occurredAt asc sorts a set already reduced to PENDING-and-due and bounded by take = AUDIT_OUTBOX_FLUSH_LIMIT (200).',
    ...
    AgentProposal:
        'listAgentProposals filters by tenantId (+status), orders by createdAt — covered by @@index([tenantId, status, createdAt]); bounded take ≤100. listQuarantinedAgentProposals (the triage surface behind /admin/mcp/quarantine) is the same shape with status pinned to QUARANTINED and is covered by the same composite — two equality columns then the sort column — which is why no guardVerdict index ships with it. Nothing filters on guardVerdict today; the query that would (FLAGGED-but-queued rows, which the reviewer queue at /agent-proposals already lists but shows no guard signal for) is not written, and its index belongs in that diff. See the note on the @@index in agentic.prisma. The two ASI09 sweeps read this model too and neither needs a new registry entry: the expiry sweep is system-wide on (status, expiresAt) and has its own non-tenant-leading @@index for exactly that reason, and the sample-audit sweep filters (tenantId, status, reviewedAt) which the same tenant-leading composite serves. Both are bounded.',
