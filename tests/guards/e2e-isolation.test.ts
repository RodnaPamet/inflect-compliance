...
const SHARED_SEED_ALLOWLIST: ReadonlyArray<{ file: string; reason: string }> = [
    ...
    { file: 'agents-views-navigation.spec.ts', reason: 'read-only: opens the agents ViewsMenu, follows every one of its entries, walks the six redirect shims and clicks the one link out of /admin/mcp. Every step is a navigation + an assertion; nothing is created, edited or deleted. Needs a tenant that EXISTS rather than an empty one — the destinations must render, and the mutating half of this feature (registering an agent) lives in agents-page.spec.ts on the isolated-tenant fixture' },
    { file: 'agentic-reports.spec.ts', reason: 'read-only: renders the agent governance pack and asserts every figure is a number or a NAMED absence, that the definitions travel with the figures, and that the two things an assessor could be misled by are on the page. Needs the shared seed — an empty tenant would report every metric as an absence and the spec would pass while proving nothing about the five real queries. The export button is asserted PRESENT and deliberately never clicked: a click files a retained Evidence row, which would make every spec that counts evidence order-dependent. The write is covered by tests/integration/agentic-reports-export.test.ts against its own tenants' },
