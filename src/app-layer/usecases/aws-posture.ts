...
export async function runAwsPostureCollection(input: {
    ...
    return runInTenantContext(ctx, async (db) => {
        ...
        try {
            ...
            await markAuthFailure(db, conn.id, e, now, 'aws-posture');
        ...
        //      ceasing to retry.
        //   2. `buildCredentialEnv` sets exactly ONE `AWS_REGION` and there is
        //      no multi-region or multi-account fan-out, so a HEALTHY
        //      credential aimed at a disabled opt-in region
        //      (steampipe-plugin-aws#75) fails every control and is
        //      indistinguishable from a rejection by breadth alone.
        //
