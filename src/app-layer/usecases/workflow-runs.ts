...
export async function resumeWorkflowRun(ctx: RequestContext, runId: string): Promise<{ status: string }> {
    ...
    const status = await executeFrom(ctx, runId, def, resumedFrom + 1, run.startedAt.getTime());
    return { status };
}
...
async function executeFrom(
    ...
    for (let seq = fromSeq; seq < def.steps.length; seq++) {
        ...
        if (runtimeHalt) return haltRunAtCap(ctx, runId, runtimeHalt, def.steps.length - seq);
        const stepHalt = budget.charge('STEPS', 1);
        if (stepHalt) return haltRunAtCap(ctx, runId, stepHalt, def.steps.length - seq);
