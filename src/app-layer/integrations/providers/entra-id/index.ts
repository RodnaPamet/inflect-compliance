...
export class EntraIdProvider implements ScheduledCheckProvider, IdentitySyncProvider {
    ...
    readonly configSchema: ConnectionConfigSchema = {
        configFields: [
            ...
            // The JOINER's opt-in, declared separately from the leaver's for
            // the reason `write-direction.ts` sets out at length: Entra's
            // consent list cannot separate the two directions, so this flag is
            // the only place the separation can be stated. Default OFF, and the
            // provisioner refuses to construct without an explicit `true` — so
            // a tenant that consents Policy.Read.All for any other reason gains
            // no ability to mint credentials here until somebody says, on this
            // connection, that they want that.
            { key: 'joinerWritesEnabled', label: 'Allow joiner credential issuance', type: 'boolean', required: false, description: 'Let joiner onboarding CREATE accounts here and mint their first credential as a Temporary Access Pass. Off by default. Additionally requires an administrator to consent the application permissions User.ReadWrite.All (to create) and Policy.Read.All (a Temporary Access Pass is governed by the authentication-methods policy, which cannot be minted without reading it). Never a password and never a password fallback: if Temporary Access Pass is disabled by policy the pass refuses, because a silent downgrade is a different security posture from the one you configured.' },
        ],
        secretFields: [
