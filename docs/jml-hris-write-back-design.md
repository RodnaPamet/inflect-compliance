...
**2026-09-13**. Each citation carries enough of the symbol name to be re-found after the next drift;
the line number is a convenience, the symbol is the evidence. Two citations in this lane's brief had
already drifted, and both are corrected in the next section — **a drifted citation is not evidence
that a claim moved.** When you edit this file, re-derive what you touch.
...
The operator's flow is: *"Joiner is created in HRIS without email … The work email created through
Entra is written back to HRIS."* A person with no work email is dropped **three independent times**,
at three different layers, before they could ever become a row this product can act on:
...
*"the only method a sync creates on its own"* (`prisma/schema/enums.prisma:1564-1567`), and the sole
creation site in `src/` writes that literal (`usecases/identity-account-link.ts:212`). `EXTERNAL_ID`
and `MANUAL` exist in the enum and are created **nowhere** — `grep -rn "'EXTERNAL_ID'" src/` returns
no `matchMethod` assignment.
...
itself is not vacuous)"*, asserting `sources.length > 500` at `:234` — which is the same empty-selection
discipline the docs ratchet needs and does not have inside its per-file loop.)
...
| Dwell | `DRY_RUN_MIN_DAYS = 7` `identity-write-policy.ts:58`, enforced in `describeRefusal` `:122` | Shipped |
...
yielded strictly less than the rung below"* (`write-ladder.ts:24-38`). Re-introducing it as a second
axis rather than a fourth rung does not make it a different mistake.
...
(`servicenow/correlation.ts:26-28`). An update addressed by the remote's own row id is already in
...
2. **The capture must be non-empty even when the finding is an absence.** `beginWrite` rejects an
   empty `priorState` (`identity-write-journal.ts:92`), and the reason it gives is exactly this
...
`disable()` contract is *"Resolves on success, throws on refusal"* (`identity-disable-account.ts:267`)
and Entra's 204 is taken as truth. Here it cannot be.
...
and a read-back confirmed it"*; this adopts that definition rather than coining another.

**And one improvement on that document.** It says `WRITEBACK_INDETERMINATE` is *"not re-attempted — a
retry against an un-queryable write is how one person gets two contact rows"*. The reasoning is
right and the conclusion is too narrow: the write is **not** un-queryable, because the read-back
exists. So the correct automatic follow-up is a **read, not a re-write**. The reconciler re-reads the
field and, if the value is there, promotes the row through the existing
`settleIndeterminateAsApplied` (`identity-write-journal.ts:527`) — a function that exists for exactly
*"a later read observed the intended end state"*. A re-write is still never automatic.
...
after a pass did something you did not expect"* (`identity-leaver-pass.ts:903-910`). The write-back
touches a **different vendor** with a **different** failure mode, and an incident in the HRIS write
must be stoppable without also stopping account creation. So: `HRIS_WRITEBACK_MAX_MODE`, initial
...
not weaker for it: the row that says *"on this date this BambooHR record's work-email field was
empty, and we put X in it"* is the only artefact that distinguishes "the product filled this" from
"HR filled this" during an access review months later. The non-empty-capture rule
(`identity-write-journal.ts:92`) is handled in Decision 3.
...
number must be able to go DOWN"* (`identity-write-breaker.ts:88-109`) — the defect that latched a
tenant's leaver path shut forever. A write-back numerator satisfies it naturally: once written, the
...
same as a safe one"* (`:154`). But the HRIS roster read is routinely incomplete by design —
...
- `appendAuditEntry`, **not** `logEvent` — a scheduled pass runs as `SYSTEM_PRINCIPAL`, which is not a
  ...
  record nothing while looking like it worked (`:1109-1121`).
...
journal id in `detailsJson`, which is already where `journalId` lives (`:1173`). This should be
revisited once the joiner settles what a pre-hire is.

Second, smaller open point: the leaver uses `category: 'access'` / `operation: 'permission_changed'`
(`:1165`, `:1167`). Writing an email into an HR record is arguably a data change, not an access change.
The recommendation is to **keep `category: 'access'`**, because the value being written is the
...
this page could render"* (`:150-155`).
...
labels (`LeaverPassesClient.tsx:600-610`). `REFUSED_NO_HANDLE` needs the same treatment — *why* there
