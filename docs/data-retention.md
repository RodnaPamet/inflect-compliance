...
| `AgentBehaviourWindow` | Operational | No | Hard-delete by `CASCADE` from `RegisteredAgent` and from the tenant | One hour of one agent's observed behaviour: call counts by capability class, the distinct tool NAMES it reached for, and the verdict its circuit breaker recorded. Counts and identifiers only — never a prompt, a proposal payload or any tool output. It is the baseline an anomaly is measured against, so a sweep that pruned it would silently return every agent to `NO_BASELINE` and stand the breaker down. **Growth is NOT bounded** — the 168-window look-back (`BASELINE_WINDOW_LIMIT`) bounds what the detector READS, not what the table stores, and rows survive until the agent or tenant is hard-deleted. Nor is that look-back seven days: a row exists only for an hour the agent was ACTIVE, so 168 windows is seven days only for an agent that calls every hour, and roughly 84 days for one active two hours a day. `baselineEpoch` is advanced by a human, so it bounds age only when somebody acts. Retention, and whether the look-back should also have an age cap, is an open question for the first deployment that runs enough agents to care — see #2461 |
