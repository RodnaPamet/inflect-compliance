...
| OrangeHRM (`orangehrm`) | Employee roster metadata — name, work email, employment status, department, manager. **Internal test fixture, not a customer-facing integration**: it exists so HRIS roster behaviour can be exercised against an HR system this team controls and can write to. Read-only OAuth2 pull; the connector's host allowlist admits only OrangeHRM-operated hosts | HRIS sync (test fixture) | OrangeHRM-operated | Token lifetime | Yes (per-tenant opt-in) |
